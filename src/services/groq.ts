...
export interface ClientConfig {
  ...
  personality: string;
  tone: string;
  formality_level: string; // 'casual' | 'professional' | 'formal'
...
export class GroqService {
  private async getClientConfig(clientId: string): Promise<ClientConfig | null> {
  ...
  private buildPrompt(config: ClientConfig, userMessage: string, context: any): string {
    const {
      personality,
      tone,
      formality_level,
    ...
    if (products_knowledge.length > 0) {
      ...
      products_knowledge.forEach((p: any) => {
    ...
    behavior_rules.forEach((r: string, index: number) => {
      prompt += `${index + 1}. ${r}\n`;
    });
    prompt += `\n`;
  ...
  async generateResponse(clientId: string, userMessage: string, context: any = {}): Promise<string> {
    try {
      ...
      const trigger = config.active_triggers.find((t: any) => 
        userMessage.toLowerCase().includes(t.trigger_word.toLowerCase())
      );
      if (trigger) {
